...
# Create the plot
fig, ax = plt.subplots()
ax.set_xlim([(-K-dw)/c - 1.5, (K-dw)/c + 2])
...
if run:
    ...
    # write binss
    with open('../simulations/binss_pos_fr.pkl', 'wb') as f:
    ...
    # write binss
    with open('../simulations/binss_neg_fr.pkl', 'wb') as f:
        # Use pickle to dump the variable binss to the file
        pickle.dump(binss_neg, f)

# read binss
with open('../simulations/binss_pos_fr.pkl', 'rb') as f:
    # Use pickle to load the variable binss from the file
    binss_pos = pickle.load(f)
with open('../simulations/binss_neg_fr.pkl', 'rb') as f:
    # Use pickle to load the variable binss from the file
...
ax.plot(duts[stable_inds],fixed[stable_inds],color='black')
ax.plot(duts[unstable_inds1],fixed[unstable_inds1],color='black',linestyle='--')
ax.plot(duts[unstable_inds2],fixed[unstable_inds2],color='black',linestyle='--')

# plot dots for saddle-node bifurcations
ax.scatter((-K-dw)/c, SN1, color='black', s=100)
ax.scatter((K-dw)/c, SN2, color='black', s=100)
...
# set labels
ax.set_ylabel(f'$\phi(t)$',fontsize=18)
ax.set_xlabel(f'$\Delta v$',fontsize=18)

# simulate the whole-system and plot evolution on figure
# model parameters
rho=1e-3;a0=1.5;ai=1.0;aii=1.0;api=0.125
eps=1.0*1e-1;c=1;delta=1*eps;w=np.array([1+dw,1]).astype(np.float64)
W = np.array([[0,1],[1,0]]).astype(np.float64)
L = np.array([[1,-1],[-1,1]]).astype(np.float64)
N = 2

# solver settings
t_span=(0,75);method='BDF';atol=1e-6;rtol=1e-3;max_step=1e-3;t_eval_sim=np.linspace(t_span[0], t_span[1], int(1500/eps))
y0=[1.0,1.0, 0.1, 0.9, 0.2, 1.15]
...
plt.scatter(dup,dtheta,color=sns_colors[3], alpha=0.1, s=3)

# REPEAT TOXIC
# simulate the whole-system and plot evolution on figure
# model parameters
rho=1e-3;a0=1.0;ai=1;aii=1;api=1.25
eps=0.75*1e-1;c=1;delta=1*eps;w=np.array([1+dw,1]).astype(np.float64)
W = np.array([[0,1],[1,0]]).astype(np.float64)
L = np.array([[1,-1],[-1,1]]).astype(np.float64)
N = 2

# solver settings
t_span=(0,10);method='BDF';atol=1e-8;rtol=1e-6;max_step=1e-4;t_eval_sim=np.linspace(t_span[0], t_span[1], int(1500/eps))
...
# plot the darn thing
colors = np.linspace(0,1,dup.size)
plt.scatter(dup,dtheta,color=sns_colors[0], alpha=0.1, s=3)

# draw vertical line at deltaut = 0
plt.axvline(x=0, color=sns_colors[1], alpha=1.0, linestyle='--', linewidth=3)


# we're done
plt.tight_layout()
plt.savefig('../plots/drift_regime.pdf', dpi=300)
plt.savefig('../plots/drift_regime.png', dpi=300)
